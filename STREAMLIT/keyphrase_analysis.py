...
class KeyphraseAnalysisInterface:
    ...
    def analyze_keyphrases_with_coreference(
        ...
        """
        print(f"DEBUG: Analyzing keyphrases for sentence: '{sentence[:50]}...'")
        print(f"  Keyphrases to analyze: {keyphrases}")
        # 1) locate or fuzzy-match the sentence in full_text
        # use spaCy to locate the exact sentence in the same text Coreferee saw
        doc = nlp(full_text)
        ...
        if best_score < 0.5:
            # fallback to literal find + tokenized fuzzy-match
            sent_start = full_text.find(sentence)
            if sent_start == -1:
                print("  SpaCy alignment low (", best_score, "), trying fuzzy match on raw text...")
        ...
        for phrase in keyphrases:
            ...
            # 5) overlap-based chain mapping (updated to direct interval test)
            matched_chain = None
            matched_span = None
            for txt, s_off, e_off in candidates:
                ...
                for chain in all_chains:
                    ...
                    if mentions and any(a < m1 and b > m0 for (m0, m1, _) in mentions):
                        ...
                        break
                if matched_chain:
                    print("matched chain:", matched_chain)
                    break

            # 6) HEAD-NOUN fallback (unchanged)
